...
- 1.0.1: dog gait from ground covered (legs had stopped moving after the real-time easing change), herder faces his direction of travel on vertical path legs, the pen's water trough drawn as a trough
...
- ROOT CAUSE of the "sea over the island" report, found by static readback at 1× on a second herder: the puddle layer computed "seconds since rain" from a last-rain tick the renderer kept across herders; a new world starts at tick 0, so wetness came out ~50 instead of ≤1 and every 23rd grass tile got an opaque blue ellipse fifteen tiles wide (overlapping into screen-width diagonal strips that shrank as the day advanced). Fixed by resetting per-world renderer timers in `setMap` (rain, hens, finale, arrivals, stick, dog) and clamping wetness to [0, 1]. The chunk-canvas hardening below was built while chasing this and is kept as defence in depth
...
- Board size 576 (was 512) so the natural pace fills the day: last sheep lands 17:52–18:12 instead of 17:24–17:36; saved 512 herders keep their own size; governor response steepened (gain 8, floor 0.4, deadband 1%) so quick boards are held to about 18:00: six seeds now land 17:56–18:33, median 18:07
