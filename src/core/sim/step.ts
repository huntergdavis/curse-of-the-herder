...
const MISHAPS: { kind: string; terrain: number[] | null; chance: number; anger: number; ticks: number }[] = [
  ...
  { kind: "dogUnderfoot", terrain: null, chance: 0.0004, anger: 11, ticks: 8 },
];
...
function stepHerder(w: WorldState, map: GameMap): void {
  ...
  if (h.mode === "toSheep") {
    ...
    if (d < FLEE_RADIUS && h.approachCount === 0) {
      ...
      if (s.flees < (s.nemesis ? MAX_FLEES + 2 : MAX_FLEES) && keyedUnit(w.seed, "flee", s.id, s.flees, w.tick) < s.skittish * 0.5 * hourFactor * grudgeFactor) {
        s.flees++;
        w.stats.flees++;
        if (w.streak >= 5) pushEvent(w, { tick: w.tick, kind: "streakBroken", sheepId: s.id, detail: String(w.streak) });
        if (s.flees === 3 && !w.sheep.some((o) => o.nemesis)) {
          s.nemesis = true;
          pushEvent(w, { tick: w.tick, kind: "nemesis", sheepId: s.id });
        }
        w.streak = 0;
        fleeTo(w, map, s);
        if (s.flees >= 2) {
